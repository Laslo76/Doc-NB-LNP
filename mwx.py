...
class MWX:
    ...
    def giv_const(self, name: str):
        ...
        if crow == 0:
            ...
            return sheet.cell(crow, 2).value

    def get_value(self, page_name='', row=0, col=0) -> str:
        result = ''
    ...
    def search_product(self, cod_: str) -> dict:
        ...
        if row != 0:
            sheet = self.book[name_sheet]
            dict_ret['код'] = cod_
            dict_ret['наименование'] = sheet.cell(row, 2).value
            dict_ret['единица'] = sheet.cell(row, 3).value
            dict_ret['ОКЕИ'] = sheet.cell(row, 4).value
        return dict_ret

    def search_nds(self, cod_: str) -> dict:
        name_sheet = 'NDS'
        dict_ret = {'наименование': '', 'ставка': 0, 'ндс_сверху': 0, 'представление': ''}
        list_search = [[1, cod_]]
        row = self.search_row(name_sheet, list_search)
        if row != 0:
            sheet = self.book[name_sheet]
            dict_ret['наименование'] = cod_
            dict_ret['ставка'] = sheet.cell(row, 3).value
            dict_ret['ндс_сверху'] = sheet.cell(row, 4).value
            dict_ret['представление'] = sheet.cell(row, 2).value
        return dict_ret

    def total_customer(self, name_page='', row=0, ret_type='all') -> str:
    ...
    def transform(self, sheet_in, row: int, name_page="", default='', tmpl='') -> str:
        result = default
    ...
    def get_str_list(self, name_list='Docs', rows=None, columns=None) -> list:
        if columns is None:
            columns = list()
        s_page = self.book[name_list]
        if rows is None:
            rows = range(2, s_page.max_row + 1)
    ...
    def prn_t12(self, row_print: int):
        page = self.book['Docs']
...
if __name__ == '__main__':
    ...
    wb.prn_t12(row_doc_print)
    wb.prn_p4(row_doc_print)
    wb.prn_sf(row_doc_print)
